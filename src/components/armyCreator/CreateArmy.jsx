...
const CreateArmy = () => {
  ...
  const [currentArmyVisibleStats, setCurrentArmyVisibleStats] = useState([]);
  ...
  const toggleStats = (unitId) => {
    setVisibleStats(prevState =>
      prevState.includes(unitId)
        ? prevState.filter(id => id !== unitId)
        : [...prevState, unitId]
    );
  };

  const toggleCurrentArmyStats = (unitId) => {
    setCurrentArmyVisibleStats(prevState =>
      prevState.includes(unitId)
        ? prevState.filter(id => id !== unitId)
        : [...prevState, unitId]
    );
    ...
    <div className="create-army-container">
      ...
      <div className="create-army-section">
        ...
        {currentArmy.map((unit, index) => (
          <div key={index}>
            <span>{unit.name}</span>
            <button onClick={() => toggleCurrentArmyStats(unit.id)}>Display Stats</button>
            {currentArmyVisibleStats.includes(unit.id) && (
              <div className="stats">
                <p>Movement: {unit.movement}</p>
                <p>Weapon Skill: {unit.weaponSkill}</p>
                <p>Ballistic Skill: {unit.ballisticSkill}</p>
